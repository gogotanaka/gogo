...
def post_with_bot(token, channel, text):
    ...
    if not resp.get("ok"):
        sys.exit(f"[mf-pl] chat.postMessage failed: {resp.get('error')}")
...
def get_cdp_tokens():
    ...
    try:
        ...
        sys.exit(
            "[mf-pl] Bot トークン設定がなく、Slack の CDP (localhost:9222) にも接続できません。\n"
...
def post_with_cdp(text):
    xoxc, xoxd = get_cdp_tokens()
    channel = os.environ.get("MF_PL_CHANNEL", "")
    if not channel:
        me = cdp_api("auth.test", {}, xoxc, xoxd)
        if not me.get("ok"):
            sys.exit(f"[mf-pl] auth.test failed: {me}")
        dm = cdp_api("conversations.open", {"users": me["user_id"]}, xoxc, xoxd)
        if not dm.get("ok"):
            sys.exit(f"[mf-pl] conversations.open failed: {dm}")
        channel = dm["channel"]["id"]
    resp = cdp_api("chat.postMessage", {"channel": channel, "text": text}, xoxc, xoxd)
    if not resp.get("ok"):
        sys.exit(f"[mf-pl] chat.postMessage failed: {resp}")
...
def main():
    ...
    if bot_token:
        channel = os.environ.get("MF_PL_CHANNEL") or read_file(CHANNEL_PATH)
        if not channel:
            sys.exit(f"[mf-pl] 投稿先チャンネルが未設定です。MF_PL_CHANNEL か {CHANNEL_PATH} で指定してください。")
