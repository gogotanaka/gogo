...
- リビッドセッション: `@bot start 3930 1000 760`（銘柄・合計株数・上限価格）で予約。
  start は**いつでも受け付け、次の8:59開始として予約する**（8:59より前に送れば当日、
  8:59以降なら翌日。土日分は翌営業日に繰り越し）。開始日の8:59に初回ティック、
  以降場中20〜30分おき（ランダム）に
...
`@bot start 3930 1000 760`（銘柄 合計株数 上限価格）でリビッドセッションを予約
（次の8:59開始）、`@bot stop` で停止・予約取消する（詳細は冒頭の「できること」参照）。
start は既存セッション・予約を置き換える。stop しても未約定の注文はそのまま残る
（消すには `clear all`）。
