...
- 板情報（気配値・本日出来高）を定期的にSlackへ投稿。`@bot book` でその場取得も可能
...
- 自動リビッド（opt-in）: `SBI_AUTO_REBID_TICKER` を設定すると、場中20〜30分おき
  （ランダム）に未約定注文を全取消し、最良買気配と同値の買い指値を300〜600株
  （ランダム、上限金額の安全弁つき）で入れ直し、毎回Slackでメンション報告する。
  `SBI_AUTO_REBID_MAX_PRICE_YEN` を設定すると、最良買気配がそれを超えている間は
  既存注文に触らず発注を見送り、超えた最初の回だけメンションで知らせる
- **売買の方針判断は人間が行う**。自動リビッドも「最良買気配に指値を置き続ける」
  という人間が決めた機械的ルールの実行のみで、相場判断はしない
...
SBI_MAX_ORDER_VALUE_YEN=500000      # メンション発注・自動リビッドの上限見積金額（円）。超えたら拒否する

# 自動リビッド（任意。SBI_AUTO_REBID_TICKER を設定した場合のみ有効）
SBI_AUTO_REBID_TICKER=3930          # 対象銘柄。場中のみ動作し、実行のたびに
                                     # 未約定注文を全取消→最良買気配と同値の買い指値を出し直す
SBI_AUTO_REBID_INTERVAL_MIN_SEC=1200  # 実行間隔の下限（秒）。既定20分
SBI_AUTO_REBID_INTERVAL_MAX_SEC=1800  # 実行間隔の上限（秒）。既定30分。毎回この範囲でランダム
SBI_AUTO_REBID_QTY_MIN=300          # 数量の下限（株）。既定300
SBI_AUTO_REBID_QTY_MAX=600          # 数量の上限（株）。既定600。毎回この範囲でランダム
SBI_AUTO_REBID_QTY_STEP=100         # 数量の刻み（売買単位）。既定100
SBI_AUTO_REBID_MAX_PRICE_YEN=780    # 発注する価格の上限（円）。最良買気配がこれを超えている間は
                                     # 発注を見送り、超えた最初の回だけメンション通知。0/未設定で制限なし
```
