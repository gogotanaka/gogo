...
`config/slack_signing_secret` か `SLACK_MENTION_USER` が無い場合、メンション機能は
無効のまま（他の機能には影響しない。判定は `mention_listener.enabled()`）。
...
- 板情報の投稿は板が動いている時間帯（前場8:00-11:30, 後場12:05-15:30, 平日のみ）に
  ...
  代わり映えしない）
- 祝日カレンダーは見ていないため、watch / watch-open も祝日に動こうとする
  （板が動かないので実発注には至らないはずだが、無駄なアクセスは発生する）
- 注文照会のページング（1日に大量の注文が溜まって複数ページになるケース）は
  未対応・実画面未確認。2ページ目以降に「注文中」があると取消・追跡から漏れる
  可能性がある。watch の取消→再発注は同時に1〜2注文しか持たない運用なので
  通常は到達しないが、大量に手動発注する日は注意
- 銘柄コードは数字4桁のみ対応（285A等の英字入りは、コマンドのパースと
  注文照会の銘柄セル解釈の両方が未対応）
- 一回きりの注文（buy/sell）には、発注の受付確認に失敗した場合の注文照会との
  突合（watch系ティックには実装済み）が無い。「発注に失敗しました」と返信された
  場合は、実際には受注されている可能性があるので注文照会を確認すること
- Slack投稿が恒久的に失敗し続ける状態（トークン失効等）を自動検知する仕組みは
  無い。報告が来なくなったと感じたら `/private/tmp/sbi-order-web.log` を確認する
  （macOS通知は独立に出続ける）
- web UI（`/` と `/orders`）はローカルからのアクセス限定（トンネル経由の
  リクエストは Cf-Ray ヘッダ等で見分けて404）。公開されるのは `/slack/events` のみ
